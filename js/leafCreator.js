...
function leafCreator() {
  ...
  myObjPromise.then((myObj) => {
    ...
    let bottomPoint = new THREE.Vector3(
      myObj.position.x + 1.65,
      myObj.position.y - 1.15,
      myObj.position.z
    );
    ...
    for (let i = 0; i < leafsPositions.length; i++) {
      ...
      newLeaf.position.set(
        leafsPositions[i][0] - newBottomPoint.x,
        leafsPositions[i][1] - newBottomPoint.y,
...
function liteLeafCreator() {
  ...
  // console.error(">>> leafShape: ", leafShape);
  ...
  console.error('leaf geometry:', leaf);
  // console.error(mesh_arr);

  // This creates leaf stem
  let stemGeometry = new THREE.CylinderGeometry(0.2, 0.6, 32, 10);
  let stemMesh = new THREE.Mesh(stemGeometry);
  // console.error('>>>leaf stemMesh: ', stemMesh);
  ...
  let leafTotal = new THREE.Mesh(leaf, material);
  ...
  let bottomPoint = new THREE.Vector3(
    leafTotal.position.x,
    leafTotal.position.y - 0.75,
    leafTotal.position.z
  );
